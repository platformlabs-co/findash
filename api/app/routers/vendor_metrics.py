...
async def get_vendor_metrics(
    ...
    try:
        logger.info(
            f"Fetching metrics for vendor: {vendor_name}, user_sub: {auth_user['sub']}"
        )
        ...
        # Calculate total sum
        total_sum = (
            sum(entry["cost"] for entry in metrics["data"])
            if metrics.get("data")
            else 0
        )
...
async def create_datadog_configuration(
    ...
    if existing_config:
        ...
        existing_config.app_key = (
            config.app_key if config.app_key else existing_config.app_key
        )
        existing_config.api_key = (
            config.api_key if config.api_key else existing_config.api_key
        )
